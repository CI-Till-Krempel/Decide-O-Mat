...
5. **Author Attribution**: "Statement von [Author Name]" label at the top of each card.
6. **Vote Button**: Heart/circle icon button on the right side of each card (replaces current "Vote (N)" pill).
7. **Voter Chips**: "Zustimmung von" (Approved by) section with name chips at the bottom of each card.
8. **Own Statement Highlight**: Cards authored by the current user have an orange/coral accent border and "Dein Statement" label.
9. **Sorting**: Statements sorted by vote count (highest first).

### Column Layout
10. **Column Headers**: "Pro hinzufuegen" / "Kontra hinzufuegen" labels with circular purple "+" buttons.
...
## Figma Reference
- Node ID: `110:1458` (Empty state — "Pro oder Con waehlen")
- Node ID: `11:21` (With statements — "Statement schreiben")
- Node ID: `110:1694` (Submitting — "Statement senden")
- Node ID: `411:1962` (Full voting — "Statement voten")
